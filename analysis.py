...
rndm = randint(0, 1460)
random_days = [i for i in demand.values()][rndm:rndm+10]
...
ax1.plot(hourly)
ax2.plot(daily_y)
ax3.plot([i for i in demand.values()])
ax4.plot(random_days)

fig.savefig('plots/demand_graphs.png')
...
with open('gurobi_files/model_sol.txt', 'r') as file:
    for result in file.readlines():
        ...
        vars.append([varname, value.replace('\n', '')])
...
fig.savefig('plots/prod_units.png')
